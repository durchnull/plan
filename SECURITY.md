...
[durchnull/plan](https://github.com/durchnull/plan/security/advisories/new) → *Report a vulnerability*.
...
`plan` reads and writes plan documents in the consuming project, and ranks them with a deterministic engine. In scope: any path where a plan document's own contents (frontmatter, a path reference, a title) cause a write outside the configured plans directory, or are executed rather than parsed.
